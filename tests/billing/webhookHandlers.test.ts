...
describe("applyTransactionCompleted — Odoo invoice creation", () => {
  function transactionEvent(overrides: Record<string, unknown> = {}) {
    // Amounts in Paddle's actual wire format: integer minor units as
    // strings (€49.00 → "4900"), per the real transaction.completed
    // payload in Paddle's docs.
    return {
      id: "txn_1",
      ...
      customData: { supabase_user_id: USER_ID },
      details: {
        totals: { subtotal: "4050", tax: "850", total: "4900", grandTotal: "4900", currencyCode: "EUR" },
      },
      ...overrides,
    };
  }

  it("calls createInvoice with the Paddle-collected total and VAT already broken out", async () => {
    const { applyTransactionCompleted } = await import("@/lib/billing/webhookHandlers");
    const supabase = new FakeSupabase();
    const calls: unknown[] = [];

    const result = await applyTransactionCompleted(supabase, transactionEvent(), async (params) => {
      calls.push(params);
      return { odooInvoiceId: 999 };
    });
    ...
    expect(supabase.tables.odoo_invoice_log.rows[0]).toMatchObject({
  ...
  it("is idempotent: does not re-invoice a transaction already marked created", async () => {
    const { applyTransactionCompleted } = await import("@/lib/billing/webhookHandlers");
    const supabase = new FakeSupabase();
    let callCount = 0;
    const createInvoice = async () => {
      callCount++;
      return { odooInvoiceId: 1 };
    };

    await applyTransactionCompleted(supabase, transactionEvent(), createInvoice);
    const result = await applyTransactionCompleted(supabase, transactionEvent(), createInvoice);

    expect(callCount).toBe(1);
    expect(result.invoiced).toBe(false);
    expect(result.reason).toMatch(/already invoiced/);
  });

  it("logs a failure and rethrows if Odoo invoice creation errors, without losing the tier update elsewhere", async () => {
    const { applyTransactionCompleted } = await import("@/lib/billing/webhookHandlers");
    const supabase = new FakeSupabase();

    await expect(
      applyTransactionCompleted(supabase, transactionEvent(), async () => {
        throw new Error("Odoo is unreachable");
      })
    ).rejects.toThrow("Odoo is unreachable");
  ...
  it("skips without throwing when there's no supabase_user_id to bill", async () => {
    const { applyTransactionCompleted } = await import("@/lib/billing/webhookHandlers");
    const supabase = new FakeSupabase();

    const result = await applyTransactionCompleted(supabase, transactionEvent({ customData: null }), async () => ({
      odooInvoiceId: 1,
    }));

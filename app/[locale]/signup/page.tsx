...
// Same shape as app/[locale]/login/page.tsx: useSearchParams() has to sit
// inside a Suspense boundary, otherwise the whole route opts out of static
// prerendering (and now that locale is a route param rather than a cookie,
// this page genuinely can be prerendered per locale).
...
function SignupForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // Absolute URL handed to Supabase, so the locale prefix has to be
        // baked in here — the locale-aware router below only applies to
        // in-app navigation.
        emailRedirectTo: `${window.location.origin}${getPathname({ href: redirectPath, locale })}`,
      },
    });
    if (error) {
      setLoading(false);
